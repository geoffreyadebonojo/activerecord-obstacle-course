...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '2. finds order id of smallest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    order_id = Order.order(:id).first.id
    # ------------------------------------------------------------
  ...
  it '3. finds order id of largest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    order_id = Order.order(:id).last.id
    # ------------------------------------------------------------
  ...
  it '5. finds multiple items by id' do
    ...
    # ----------------------- Using Ruby -------------------------
    # items = Item.all.select { |item|  binding.pry; ids.include?(item.id) }
    # ------------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = User.find(2).orders.joins(:order_items).where('order_items.item_id = ?', item_4.id)
    # -----------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).select('users.*, count(orders.id) as total_order_count').group(:user_id).order(:name)
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    User
    .joins(:order_items)
    .select('users.*, count(order_items.id) as total_item_count')
    .group(:user_id)
    .order(name: :desc)
    # ---------------------------------------------------------------
